...
## Два окружения одновременно

Тестовый и боевой серверы рассчитаны на одновременную работу: разные порты,
разные базы, разные снимки frontend. Планшет может быть подключён к любому из
них — сертификат общий.

| | тестовое | боевое |
|---|---|---|
| запуск | `start.bat` (корень) | `prod\start.bat` |
| адрес | `https://<IP>:8444` | `https://<IP>:8443` |
| база | `data\vet-test.db` | `prod\data\vet.db` (живая) |
| frontend | `frontend\` (правится напрямую) | `prod\frontend\` (снимок) |
| ENV | `development` | `production` |

Обновить боевое окружение из исходников: `build-prod.bat` — собирает
`backend.exe`, копирует его и снимок frontend в `prod\`. Живая база не
затрагивается. Перед запуском остановите боевой сервер, иначе `backend.exe`
занят и копирование не пройдёт.

### Сертификат

Сертификат один на оба сервера: он привязан к именам и IP машины, а не к порту.
Открытие второго порта само по себе ничего не ломает — но если у машины
появился новый IP (сменилась сеть, включился VPN), в сертификате его нет, и
планшет ругается на «недоверенный сертификат».

```bash
go run ./scripts/gen_cert/
```

Скрипт перевыпускает серверный сертификат под текущие адреса, переиспользуя
корневой CA (`rootCA.pem`), — переустанавливать корень на планшетах не нужно.
Копия для боевого сервера кладётся в `prod\data` автоматически. После
перевыпуска перезапустите оба сервера.
...
|------------|-------------|----------|
| `PORT` | `8080` | Порт сервера (8444 — тест, 8443 — бой) |
| `TLS_CERT` / `TLS_KEY` | пусто | Пути к сертификату и ключу. Пусто — режим HTTP (только localhost, Service Worker на планшете не заработает) |
