...
(function () {
  ...
  function editorHTML() {
    ...
    return '<div class="form-grid">'
      ...
      + t.fields.length + ')</div>'
      // U27. Правка бланка со стороны выглядит так, будто она задним числом
      // меняет уже внесённые результаты. Это не так — каждый результат хранит
      // копию бланка на момент заполнения (правило 6), — но знать об этом
      // человеку неоткуда, и он либо боится править, либо заводит бланк-дубль.
      + (t.id ? '<div class="form-hint mb-2">Правка не затронет уже внесённые '
              + 'результаты: каждый хранит бланк на момент заполнения.</div>' : '')
