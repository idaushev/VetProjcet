(function () {
  ...
  async function runSync() {
    ...
    try {
      ...
      // минуту на каждом планшете, и это же грузило разбор на слабом железе.
      // Инкрементальный pull был написан в TECH-001, но цикл его не вызывал:
      // pullSync остался только в syncAll, которым цикл не пользуется.
      //
      // Раз в 20 циклов (≈5 минут) — всё-таки полный снимок: страховка от
      // записи, потерянной из-за расхождения часов. Дёшево и снимает риск
      // «запись есть на сервере, но её никто больше не увидит».
      _syncTick++;
      var pullResult = (_syncTick % 20 === 0)
        ? await window.VetSync.pullFull()
        : await window.VetSync.pullSync();
      ...
      // B-013: сервер не принял часть записей — они остаются на планшете
      // и уходят в каждый следующий push. Молчать об этом нельзя: так
      ...
      // Окончательный отказ (нет права): правка на сервер не попадёт и при
      // следующем обновлении уступит серверной версии. Сказать сразу —
      // иначе она исчезнет у врача на глазах без объяснения.
      if (pushResult.dropped && pushResult.dropped.length && window.VetUI && window.VetUI.toast) {
        window.VetUI.toast("Сервер не принял " + pushResult.dropped.length +
          " правк. — нет права на запись. Будет показана версия с сервера.", "warn", 10000);
      }
  ...
  async function createEntity(storeName, body) {
    ...
    // Способ учёта по ТАҢБА: сервер выводит 'chip' из непустого номера
    // (normalizeIDMethod). Дублируем, иначе животное, заведённое офлайн,
    // до первой синхронизации выглядит «без способа учёта».
    if (storeName === "pets" && !body.id_method) body.id_method = body.chip_number ? "chip" : "";
    if (storeName === "pets" && body.sterilized === undefined) body.sterilized = 0;
