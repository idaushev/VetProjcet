...
(function () {
  ...
  async function renderVisitVaccinations(visitId, petId) {
    ...
    if (!saved.length && !pend.length) {
      // Пусто и так видно по счётчику в шапке — строки не рисуем (U25).
      // Назначения ниже — исключение: там подсказка указывает на другое поле.
    } else {
      html += '<div class="attach-list">';
  ...
  async function renderAttachments(visitId) {
    ...
    if (!saved.length && !queued.length) {
      // U25. «Сканов и снимков пока нет» под шапкой «ВЛОЖЕНИЯ 0» с кнопкой
      // «Добавить» ничего не сообщает: три таких блока подряд на форме приёма
      // съедали треть экрана рассказом о том, чего нет. Оставляем строку
      // только там, где она УЧИТ чему-то, — у несохранённого приёма врач не
      // знает, можно ли прикладывать файл до сохранения.
      if (isDraftVisitKey(visitId)) {
