...
(function () {
  ...
  async function mergePulledStore(storeName, remoteRecords) {
    ...
    remoteRecords.forEach(function (remote) {
      ...
      if (remote.is_deleted === 1 || remote.is_deleted === true) {
        // ТОЛЬКО если запись у нас действительно есть. Сервер шлёт надгробия
        // удалённых записей в каждом снимке — вечно. Раньше клиент послушно
        // удалял их каждый цикл, хотя удалять было нечего, и считал это
  ...
  async function syncAll() {
    ...
    // 3. Pull снова: сервер принял изменения — забираем подтверждение
    //    (is_deleted=1, серверные updated_at, авторство).
    //
    //    TECH-001. Здесь стояла ПОЛНАЯ выгрузка (pullFull без since) — «чтобы
    //    гарантированно не пропустить удалённые». Планшет перекачивал всю базу
    //    после каждого цикла с изменениями: трафик, батарея, износ флеш-памяти.
    //
    //    Проверка показала, что удаления инкрементальный pull возвращает
    //    исправно (тест TestIncrementalPullReturnsDeletion) — полная выгрузка
    //    прикрывала не их, а ошибку границы since: доля секунды сериализовалась
    //    с переменным числом знаков, и запись, сделанная сразу после отметки
    //    времени, оказывалась «строкой раньше» неё и терялась
    //    (TestIncrementalPullDoesNotLoseRecordsAtBoundary). Причина устранена
    //    на сервере, поэтому обычного инкрементального pull достаточно.
    if (pushed.pushed > 0 || pushed.fallback) {
      try { await pullSync(); } catch(e) { /* не критично */ }
    }
