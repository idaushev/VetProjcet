...
//
// Доля секунды — ВСЕГДА три знака (.000, а не .999). Формат «.999» отбрасывает
// нули, из-за чего длина строки плавает, а SQLite сравнивает времена как ТЕКСТ.
// «…10.5Z» короче «…10.503Z», и на месте расхождения стоит 'Z' (0x5A) против
// цифры — то есть более РАННЕЕ время оказывается строкой БОЛЬШЕ более позднего.
// Инкрементальный pull на этом терял и лишний раз слал записи; тест
// TestIncrementalPullCoversSeveralStores ловил это примерно раз из трёх.
// Фиксированная ширина делает текстовый порядок равным хронологическому.

type sqlTime time.Time

func (t sqlTime) Value() (driver.Value, error) {
	return time.Time(t).UTC().Format("2006-01-02T15:04:05.000Z07:00"), nil
}

// T — время для SQL-запроса.
func T(t time.Time) sqlTime { return sqlTime(t) }

// sqlSince — ГРАНИЦА инкрементального pull (WHERE updated_at >= ?).
//
// Формат тот же, что у sqlTime (три знака доли секунды), но тип отдельный
// намеренно: это точка, где задокументированы правила границы, и менять
// формат времени в базе, не подумав о ней, не даст перепутать.
//
// Записи, сделанные ровно в граничную миллисекунду, приезжают повторно —
// сравнение включающее. Это безобидно: слияние на клиенте идемпотентно, а
// терять записи нельзя. Строгое «>» такую запись пропускало молча.
//
// Старые строки, записанные форматом «.999» до этой правки, могут оказаться
// строкой больше равной им границы и приехать лишний раз — тоже безобидно и
// самоустраняется при первой же их перезаписи.
type sqlSince time.Time

func (t sqlSince) Value() (driver.Value, error) {
	return time.Time(t).UTC().Format("2006-01-02T15:04:05.000Z07:00"), nil
}

// S — граница «начиная с» для инкрементального pull.
//
// Сравнение в запросах ВКЛЮЧАЮЩЕЕ (>=), а не строгое. Причина: клиент
// запоминает server_time предыдущего ответа, а запись, сделанная в ТУ ЖЕ
// миллисекунду, получает ровно это значение updated_at — при строгом «>» она
// молча выпадала из выдачи. Тест TestIncrementalPullReturnsDeletion ловил это
// примерно раз из десяти прогонов. Повторная отправка записей граничной
// миллисекунды безвредна: слияние на клиенте идемпотентно, и таких записей
// обычно ноль или единицы.
func S(t time.Time) sqlSince { return sqlSince(t) }
