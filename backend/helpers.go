...
//
// Отличается от sqlTime одним: доля секунды всегда ТРИ знака. В формате
// «.999» Go отбрасывает нули, и граница 190 мс превращается в «…12.19Z».
// SQLite сравнивает такие значения как текст, а 'Z' (0x5A) больше любой
// цифры — поэтому запись «…12.193Z», сделанная ПОЗЖЕ границы, оказывалась
// строкой МЕНЬШЕ её и в инкрементальный pull не попадала. Планшет молча
// терял запись до следующей полной выгрузки.
//
// Фиксированные три знака делают текстовый порядок равным хронологическому.
// Хранимые значения со «срезанными» нулями («…12.19Z») при этом оказываются
// строкой БОЛЬШЕ равной им границы и приезжают повторно — это безобидно:
// слияние идемпотентно, а терять записи нельзя.
