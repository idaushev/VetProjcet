...
func normalizeAndValidatePatientCondition(v string) (string, error) {
	...
	switch n {
	case "здоров", "лёгкое", "средней тяжести", "тяжелое", "крайне тяжелое", "терминальное",
		// обратная совместимость со старым значением
		"удовлетворительное":
		return n, nil
	default:
		return "", errors.New("patient_condition: здоров | лёгкое | средней тяжести | тяжелое | крайне тяжелое | терминальное")
	}
}
